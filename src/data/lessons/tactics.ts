...
const fork: Lesson = {
  ...
  objectives: ['認得捉雙：一子攻兩子', '知道馬的捉雙最難防（不能被擋）', '知道兵也能捉雙'],
  playerColor: 'white',
  steps: [
    {
      fen: 'r3k3/8/8/1N6/8/8/8/4K3 w - - 0 1',
      text: '先看清楚整個盤面：黑王在 e8，黑車在 a8。有沒有一個格子，能讓你的馬「同時」碰到這兩個子？',
      highlights: ['e8', 'a8'],
    },
    {
      fen: 'r3k3/8/8/1N6/8/8/8/4K3 w - - 0 1',
      text: '找到那個「一石二鳥」的落點，馬跳上去。',
      arrows: [
        { orig: 'b5', dest: 'c7' },
        { orig: 'c7', dest: 'e8' },
        { orig: 'c7', dest: 'a8' },
      ...
      hint: '逐一想像馬能跳到的格子，哪一個能同時搆到 e8 的王「和」a8 的車？而且若那一步順便將軍，對手就更沒辦法兼顧了。',
      successText: '看到沒？這是「將軍」捉雙：對手必須先救王，你下一步就白吃車。馬的捉雙特別致命——因為馬的攻擊沒辦法被擋住。',
    },
    {
      fen: '4k3/8/3n1n2/8/4P3/8/8/4K3 w - - 0 1',
      text: '捉雙不是馬的專利。看這兩匹黑馬並排在 d6、f6——你的兵動一步會發生什麼事？',
      highlights: ['d6', 'f6'],
    },
    {
      fen: '4k3/8/3n1n2/8/4P3/8/8/4K3 w - - 0 1',
      ...
      hint: '兵靠斜線吃子。把兵往前推一格之後，它左右兩條斜前方會不會正好各站著一匹馬？',
      successText: '一個小兵同時威脅兩匹馬——對手只能救一匹。捉雙的本質不是子力大小，而是「同時攻擊」這個圖案，任何子都能做到。',
    },
    {
...
const pin: Lesson = {
  ...
  scenario: '對手的一匹馬擋在你和它的王中間。如果那匹馬動了，後面的王就暴露了——這代表那匹馬其實「動不了」。怎麼利用？',
  objectives: ['認得牽制：前子不能動，因為身後有更重要的子', '理解「絕對牽制」（身後是王，依規則不能動）'],
  playerColor: 'white',
  steps: [
    {
      fen: '4k3/8/8/4n3/8/8/8/R5K1 w - - 0 1',
      text: '黑馬在 e5，黑王在它正後方的 e8。想想看：如果你的車站到 e 線上，那匹馬會變怎樣？',
      highlights: ['e5', 'e8'],
    },
    {
      fen: '4k3/8/8/4n3/8/8/8/R5K1 w - - 0 1',
      ...
      hint: '馬和牠的王連成一條直線。你要的不是吃馬，而是用車「壓住」整條線——哪一步能做到？',
      successText: '這就是「絕對牽制」：馬一旦移開，車就將軍它的王——依規則它根本不能動。被釘死的子等於暫時消失，你可以從容地多派子力來吃掉它。',
    },
    {
      fen: '4k3/8/8/4n3/8/8/8/4R1K1 b - - 0 1',
      text: '牽制的價值在於「凍結」對手的一個子。下次看到對手某個子和它的王（或后、車）連成一線，就想到牽制——把那條線佔住。',
    },
  ],
}
  ...
  steps: [
    {
      ...
      text: '黑王在 e5，同一橫排的 a5 站著黑車。王是最值錢的，它一定得對將軍讓步。你的車能不能利用這條橫線？',
      highlights: ['e5', 'a5'],
    },
    {
      fen: '8/8/8/r3k3/8/8/8/4K2R w - - 0 1',
      ...
      hint: '王和車在同一排上。你要從這一排的哪一端、用車將軍黑王，逼牠讓開、把後面的車暴露出來？',
      successText: '王被將軍，只能讓開這條橫線——然後你就吃掉它身後的車。這就是串擊：把「大子」逼到前面當盾牌，後面的子就保不住了。記住和牽制的差別：牽制是後面大、串擊是前面大。',
    },
    {
...
const discoveredAttack: Lesson = {
  ...
  scenario: '你的車和對手的王在同一條線上，但中間卡著你自己的一匹馬。這匹馬擋住了車——但反過來想，移開它的瞬間會發生什麼？',
  objectives: ['認得閃擊：移開一子、露出另一子的攻擊', '理解「閃將」最強：一步同時做兩件事'],
  playerColor: 'white',
  steps: [
    {
      fen: '4k3/8/3q4/8/4N3/8/8/4R1K1 w - - 0 1',
      text: '你的車在 e1、黑王在 e8，但馬卡在 e4 中間。同時注意：黑后就站在 d6。你的馬一動，會同時發生兩件事——是哪兩件？',
      highlights: ['e1', 'e8', 'd6'],
    },
    {
      fen: '4k3/8/3q4/8/4N3/8/8/4R1K1 w - - 0 1',
      text: '讓馬一步做兩件事：自己賺一個子，同時替後面的車開路。',
      arrows: [
        { orig: 'e4', dest: 'd6' },
        { orig: 'e1', dest: 'e8' },
      ],
      expectedMove: { from: 'e4', to: 'd6' },
      hint: '馬一離開 e 線，車就直接打到黑王。那麼，要往哪裡跳，才能在開路的同時順手撈走最值錢的黑后？',
      successText: '一步兩得！馬吃了后，同時車的 e 線打開、將軍黑王。對手忙著解將，根本來不及救后——你白賺一個后。這就是閃擊的威力：移開的那個子自己也在做事。',
    },
    {
  ...
  steps: [
    {
      ...
      text: '黑象從 b4 沿斜線盯上你 e1 的車。被攻擊時你有四招：移開、保護、反擊、或吃掉攻擊者。這裡哪一招最乾脆？',
      highlights: ['b4', 'e1'],
    },
    {
      fen: '4k3/8/8/8/1b6/P7/8/4R1K1 w - - 0 1',
      ...
      hint: '威脅來自那隻象。你身邊有沒有一個小子，剛好能把攻擊你的那隻象吃掉？用小換大幾乎都划算。',
      successText: '威脅來源直接消失，最省事。能用小子吃掉攻擊你大子的子，幾乎永遠划算。',
    },
    {
      fen: '4k3/8/8/3R4/8/8/b3N3/4K3 w - - 0 1',
      text: '這次不同：黑象從 a2 盯上你 d5 的車，但你沒有子能吃到它。除了逃，還能怎麼守住車？',
      highlights: ['a2', 'd5'],
    },
    {
      fen: '4k3/8/8/3R4/8/8/b3N3/4K3 w - - 0 1',
      text: '不一定要逃——給你的車找一個後盾。',
      arrows: [{ orig: 'e2', dest: 'f4' }],
      expectedMove: { from: 'e2', to: 'f4' },
      hint: '逃不掉、也吃不到攻擊者時，第三條路是「加一個防守者」。哪個馬步能讓馬保護到 d5？這樣對手吃車你就能吃回來。',
      successText: '現在車有了後盾：對手若 Bxd5，你就 Nxd5 換回來，不虧。這叫「加一個防守者」——不一定要逃，站穩也是一種解法。',
    },
    {
  ...
  steps: [
    {
      ...
      text: '先別急著走。整條 e 線上，前面站著最值錢的黑王、後面是黑后——這正是「串擊」的圖案：逼前面的王讓開，後面的后就保不住了。你的車該去哪？',
      highlights: ['e4', 'e7'],
    },
    {
      fen: '8/4q3/8/8/4k3/8/8/R5K1 w - - 0 1',
      ...
      arrows: [{ orig: 'a1', dest: 'e1' }],
      expectedMove: { from: 'a1', to: 'e1' },
      hint: '王和后都在 e 線上、王在前面。哪一步能讓你的車佔住整條 e 線、正面將軍黑王，逼牠讓出這條線？',
      successText: '將軍！黑王被迫離開 e 線——它一讓開，後面的后就失去掩護了。',
    },
    {
      fen: '8/4q3/8/3k4/8/8/8/4R1K1 w - - 0 1',
      text: '黑王逃到 d5（我替對手走了這步）。后孤零零地站在 e7，而你的車正控制著整條 e 線。收網吧。',
      highlights: ['e7'],
    },
    {
      fen: '8/4q3/8/3k4/8/8/8/4R1K1 w - - 0 1',
      ...
      hint: '黑王已被趕離 e 線，后孤零零留在線上、無人保護。你佔線的車能一路走到哪裡？',
