...
const rookAndBishop: Lesson = {
  ...
  objectives: ['知道車沿橫列與直行移動', '知道象沿斜線移動'],
  playerColor: 'white',
  steps: [
    {
      fen: '7k/8/8/8/3R4/8/8/4K3 w - - 0 1',
      text: '這是「車」。它沿著橫列或直行，能走任意多格——只要路上沒被擋住。它是棋盤上的長程砲。',
      highlights: ['d4'],
    },
    {
      fen: '7k/3p4/8/8/3R4/8/8/4K3 w - - 0 1',
      text: '盤面上有個黑兵可以拿。先看車能罩到哪些格，再決定怎麼走。',
      arrows: [{ orig: 'd4', dest: 'd7' }],
      expectedMove: { from: 'd4', to: 'd7' },
      hint: '車只走直線（橫或直）。從它現在的位置沿著線看出去，哪條線盡頭有東西可吃？',
      successText: '對！車就是直來直往的遠程火力，一條暢通的線就是它的高速公路。',
    },
    {
      fen: '7k/8/8/8/8/8/8/2B1K3 w - - 0 1',
      text: '這是「象」。它只能沿斜線走，所以它一輩子待在同一種顏色的格子上——這是象的天生限制。',
      highlights: ['c1'],
    },
    {
      fen: '7k/8/7p/8/8/8/8/2B1K3 w - - 0 1',
      text: '遠方有個黑兵。順著象能走的方向看出去，它在你的射程內嗎？',
      arrows: [{ orig: 'c1', dest: 'h6' }],
      expectedMove: { from: 'c1', to: 'h6' },
      hint: '象只走斜線。沿著它所在格子的斜對角線一路看到底，盡頭站著誰？',
      successText: '很好！車管直線、象管斜線——兩者合起來就能罩住整個棋盤。',
    },
    {
      fen: '7k/8/7B/8/8/8/8/4K3 b - - 0 1',
      text: '車是十字、象是叉叉。腦中有了這兩張「勢力範圍圖」，你看盤面就會快很多。',
    },
  ],
}
...
const knightAndQueen: Lesson = {
  ...
  tier: 1,
  order: 3,
  summary: '馬走「日」字並能跳過棋子，后是車加象的合體。',
  scenario: '一個最古怪、一個最強大。馬會跳、后全能——把它們的個性看懂，比背走法有用得多。',
  objectives: ['知道馬走 L 形且可跳子', '知道后能直能斜、是最強子力'],
  playerColor: 'white',
  steps: [
    {
      fen: '7k/8/8/8/3N4/8/8/4K3 w - - 0 1',
      text: '這是「馬」。它走「日」字形（先直走兩格、再橫一格），而且是唯一能「跳過」其他棋子的子——它不怕被擋路。',
      highlights: ['d4'],
    },
    {
      fen: '7k/8/4p3/8/3N4/8/8/4K3 w - - 0 1',
      text: '有個黑兵在 e6。馬的走法很反直覺，慢慢推算哪一步能跳到它身上。',
      arrows: [{ orig: 'd4', dest: 'e6' }],
      expectedMove: { from: 'd4', to: 'e6' },
      hint: '把 e6 當終點反過來想：哪些格子能用「日」字跳到 e6？你的馬正好站在其中一個上嗎？',
      successText: '對了！馬的跳躍最適合出其不意，因為它能無視中間擋路的子。',
    },
    {
      fen: '7k/8/8/8/8/8/8/3QK3 w - - 0 1',
      text: '這是「后」。它等於「車 + 象」：直線、斜線都能走任意多格，是棋盤上最強的子力——也因此最珍貴，別輕易讓它涉險。',
      highlights: ['d1'],
    },
    {
      fen: '7k/8/8/8/p7/8/8/3QK3 w - - 0 1',
      text: '遠處 a4 有個黑兵。后有兩種「身分」，這次用它像象的那一面去拿。',
      arrows: [{ orig: 'd1', dest: 'a4' }],
      expectedMove: { from: 'd1', to: 'a4' },
      hint: '后能走直線也能走斜線。a4 跟你的后不在同一橫直線上——那它們之間是什麼關係？',
      successText: '威風！后既能當車也能當象。正因為它什麼都能做，失去它幾乎等於輸掉一半。',
    },
    {
      fen: '7k/8/8/8/Q7/8/8/4K3 b - - 0 1',
      text: '馬靠跳躍製造意外、后靠全能掌控全局。主要子力的「個性」你都認識了。',
    },
  ],
}
...
const kingAndValue: Lesson = {
  ...
  objectives: ['知道王每步走一格', '記住子力價值：兵1、馬象3、車5、后9'],
  playerColor: 'white',
  steps: [
    {
    ...
    },
    {
      ...
      text: '每種子有大概的「身價」：兵=1、馬=3、象=3、車=5、后=9，王則是無價（丟了就輸）。換子時，心裡要有這把尺。',
    },
    {
      fen: '7k/8/8/1p3r2/3N4/8/8/4K3 w - - 0 1',
      text: '你的馬同時能吃 b5 的兵或 f5 的車。兩個都吃得到——但別急，先想哪個選擇對你更有利。',
      arrows: [{ orig: 'd4', dest: 'f5' }],
      highlights: ['b5', 'f5'],
      expectedMove: { from: 'd4', to: 'f5' },
      hint: '兩個目標都在馬的攻擊範圍內。回想剛才的身價表——拿走哪一個，對手會更心痛？',
      successText: '聰明！同樣一步，賺 5 分的車遠勝賺 1 分的兵。永遠優先換取價值更高的子。',
    },
    {
  ...
  steps: [
    {
      ...
      text: '白車正沿著 e 線攻擊黑王——這就是「將軍 (check)」。被將軍的一方，下一步「只能」處理這件事，別無選擇。',
      highlights: ['e8', 'e1'],
    },
    {
      fen: '4k3/8/8/8/8/8/8/R5K1 w - - 0 1',
      text: '換你來將軍黑王。黑王站在 e 線上——你手上的車能怎麼利用這條線？',
      arrows: [{ orig: 'a1', dest: 'e1' }],
      expectedMove: { from: 'a1', to: 'e1' },
      hint: '將軍 = 你的子攻擊到對方的王。哪一步能讓你的車控制住黑王所在的整條直線？',
    ...
    },
    {
      ...
      text: '現在輪到你的白王被黑象將軍了。三招之中，哪一招在這裡最乾脆？',
      arrows: [{ orig: 'e1', dest: 'f2' }],
      expectedMove: { from: 'e1', to: 'f2' },
      hint: '移王、擋子、吃掉攻擊者。攻擊你的那隻象就在王的旁邊——有沒有辦法一勞永逸地除掉威脅來源？',
...
const specialRules: Lesson = {
  ...
  tier: 1,
  order: 7,
  summary: '王車易位、吃過路兵、兵的升變、逼和——四個容易忽略的特殊規則。',
  scenario: '四個新手常忽略的規則。每一個背後都有它存在的「理由」，懂了理由就不必硬背。',
  objectives: ['會王車易位', '會吃過路兵', '會把兵升變', '認識逼和（和棋）'],
  playerColor: 'white',
  steps: [
    {
      fen: '4k3/8/8/8/8/8/8/4K2R w K - 0 1',
      text: '「王車易位」是唯一能同時動兩個子的走法。它的目的很單純：快速把王送到角落、變安全，同時讓車參戰。',
      highlights: ['e1', 'h1'],
    },
    {
      fen: '4k3/8/8/8/8/8/8/4K2R w K - 0 1',
      text: '做一次短易位。動的是王（車會自動跟上）——想想王該往哪個方向移、移幾格。',
      arrows: [{ orig: 'e1', dest: 'g1' }],
      expectedMove: { from: 'e1', to: 'g1' },
      hint: '易位時你移動的是「王」，朝著車的方向跨兩格。車會自己跳到王的另一側。',
      successText: '易位完成！王躲好了、車也順勢參戰。開局盡早易位，是保護王的好習慣。',
    },
    {
